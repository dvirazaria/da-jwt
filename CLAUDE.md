...
Keep one source of truth: the `state` object in `kupa-sgura.html`. The UI route is `appView` (`friends | games | game | settle | profile | group`); persisted game phase is `state.phase` (`active | settlement | closed`). `friends`, `games`, and `profile` are primary bottom-nav screens. `game`, `settle`, and `group` (a single group's own page, selected by the UI-only `currentGroupId`) are contextual screens. On boot, an open game resumes at its active/settlement phase; otherwise the app opens Profile. Only final `סגור שולחן` closes the game, creates history/debts, and returns to Games (or, for a group game, to that group's page).
