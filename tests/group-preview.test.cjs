...
test('the dashboard group preview is a full in-app overlay with one injected close control', () => {
  ...
  assert.match(html, /<div class="group-sheet" id="groupPreview" role="dialog" aria-modal="true" aria-label="[^"]*" hidden>/);
...
test('opening a preview reuses the existing group adapters and renderers', () => {
  ...
  const renderer = sourceBetween('  function renderGroupPreview() {', '  function renderGroupHeader(');
